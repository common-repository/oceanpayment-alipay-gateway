...
/**
 * Oceanpayment AliPay Payment Gateway
 *
 * Provides a Oceanpayment AliPay Payment Gateway, mainly for testing purposes.
 *
 * @class 		WC_Gateway_Oceanalipay
 * @extends		WC_Payment_Gateway
 * @version		1.4
 * @package		WooCommerce/Classes/Payment
 * @author 		Oceanpayment
 */
class WC_Gateway_Oceanalipay extends WC_Payment_Gateway {
    ...
    public function __construct() {
        $this->id                 = 'oceanalipay';
        $this->icon               = apply_filters('woocommerce_oceanalipay_icon', plugins_url( 'images/op_Alipay.png', __FILE__ ));
        $this->has_fields         = false;
        $this->method_title       = __( 'Oceanpayment AliPay', 'oceanpayment-alipay-gateway' );
        $this->method_description = __( '', 'oceanpayment-alipay-gateway' );
        $this->supports           = [
            'products',
//            'tokenization',
//            'add_payment_method',
        ...
        $this->Title = $this->get_option('_oceanalipay_title') ? $this->get_option('_oceanalipay_title') : '';
        $this->Body = $this->get_option('_oceanalipay_description') ? $this->get_option('_oceanalipay_description') : '';

        // Actions
        add_action( 'woocommerce_api_wc_gateway_oceanalipay', array( $this, 'check_ipn_response' ) );
        add_action( 'woocommerce_update_options_payment_gateways_' . $this->id, array( $this, 'process_admin_options' ) );
        add_action( 'valid-oceanalipay-standard-itn-request', array( $this, 'successful_request' ) );
        add_action( 'woocommerce_receipt_' . $this->id, array( $this, 'receipt_page' ) );
        add_action( 'woocommerce_thankyou_oceanalipay', array( $this, 'thankyou_page' ) );
...
//        add_action( 'woocommerce_receipt_oceanalipay', array( $this, 'oceanalipay_receipt_page' ) );
    }
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type'    => 'checkbox',
                'label'   => __( 'Enable Oceanpayment AliPay Payment', 'woocommerce' ),
                'default' => 'yes'
            ),
            'title' => array(
                ...
                'default'     => __( 'AliPay Payment', 'woocommerce' ),
                'desc_tip'    => true,
            ),
            'description' => array(
	...
	public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
		if ( $this->instructions && ! $sent_to_admin && 'oceanalipay' === $order->get_payment_method() && $order->has_status( 'on-hold' ) ) {
     ...
     * 生成 AliPay form.
     */
    public function generate_alipay_form( $order_id ) {
        ...
        //账单人州(可不提交)
        $billing_state     = $this->get_alipay_state( $order->get_billing_country(), $order->get_billing_state() );
        //账单人城市
        ...
        //收货人的州（省、郡）
        $ship_state 	   = empty($this->get_alipay_state( $order->get_shipping_country(), $order->get_shipping_state())) ? $billing_state : $this->get_alipay_state( $order->get_shipping_country(), $order->get_shipping_state());
        //收货人的城市
        ...
        $data_to_send .= "</div>";
        $data_to_send .= "</div>";
        $data_to_send .= "<form  method='post' name='alipay_checkout' action='".$this->settings['submiturl']."'  >";
        ...
        $data_to_send .= 'document.alipay_checkout.submit();' . "\n";
        $data_to_send .= '</script>' . "\n";
            ...
            if($return_info['response_type'] == 1){
                ...
                if(strtoupper($local_signValue) == strtoupper($return_info['signValue'])){
                    ...
                    if ($return_info['payment_status'] == 1) {
                        //成功
                        $order->update_status( 'processing', __( $testorder.$return_info['payment_details'], 'oceanpayment-alipay-gateway' ) );
                        ...
                        if(empty($this->completed_orders()) || !in_array($return_info['order_number'], $this->completed_orders())){
                    		$order->update_status( 'on-hold', __( $testorder.$return_info['payment_details'], 'oceanpayment-alipay-gateway' ) );
                		}
                        ...
                        if($errorCode == '20061'){
                        	...
                        	if(empty($this->completed_orders()) || !in_array($return_info['order_number'], $this->completed_orders())){
                    			$order->update_status( 'failed', __( $testorder.$return_info['payment_details'], 'oceanpayment-alipay-gateway' ) );
                			}
                        }
    ...
    function return_payment( $order ) {
        ...
        if(strtoupper($local_signValue) == strtoupper($back_signValue)){
            ...
            if ($payment_status == 1) {
                //成功
                $order->update_status( 'processing', __( $testorder.$payment_details, 'oceanpayment-alipay-gateway' ) );
                WC()->cart->empty_cart();
                ...
                if(empty($this->completed_orders()) || !in_array($_REQUEST['order_number'], $this->completed_orders())){
                	$order->update_status( 'on-hold', __( $testorder.$payment_details, 'oceanpayment-alipay-gateway' ) );
        		}
                $url = $this->get_return_url( $order );
                wc_add_notice( $testorder.$payment_details, 'success' );
                ...
                if($errorCode == '20061'){
					...
					if(empty($this->completed_orders()) || !in_array($_REQUEST['order_number'], $this->completed_orders())){
                		$order->update_status( 'failed', __( $testorder.$payment_details, 'oceanpayment-alipay-gateway' ) );
        			}
                    $url = esc_url( wc_get_checkout_url() );
     ...
     * 获取州/省
     */
    public function get_alipay_state( $cc, $state ) {
